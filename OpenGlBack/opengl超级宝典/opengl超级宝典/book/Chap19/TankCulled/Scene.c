...
#define RADTODEG(x) ((x)*57.2957795130823229)
...
void RenderScene(void)
	...
	// Translate the Frustum center by the camera translation
	fCenterX = cameraData.position[0];
	fCenterZ = -cameraData.position[2];

	// Extend the point outward
	fCenterX += sin(DEGTORAD(cameraData.orientation[1]))*25.0;
	fCenterZ -= cos(DEGTORAD(cameraData.orientation[1]))*25.0;

	for(nModel = 0; nModel < (MODEL_COUNT * 3); nModel++)
		{
		// If the model center is outside the sphere/frustum eliminate it
		if( ((fCenterX - pModelList[nModel].position[0])*(fCenterX - pModelList[nModel].position[0])
			+ (fCenterZ - pModelList[nModel].position[2])*(fCenterZ - pModelList[nModel].position[2]))
			> (30.0 * 30.0))
			{
			nCulled ++;
			continue;
