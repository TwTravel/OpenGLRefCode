/*
 * Terrain example 11 from Chapter 18.
 *
...
main(int  argc,    /* I - Number of command-line arguments */
    ...
    HDC                   printer, /* Printer device context */
                          dc;      /* Memory device context */
    ...
    if (!PrintDlg(&pd))
        {
	puts("Printing cancelled!");
	return (0);
	}

    /* Set the bitmap size to 1/16 the actual (1/4 the resolution) */
    Width  = GetDeviceCaps(pd.hDC, HORZRES) / 4;
    Height = GetDeviceCaps(pd.hDC, VERTRES) / 4;
    ...
    doc.lpszDocName = "Off-Screen Terrain";
    doc.lpszOutput  = NULL;

    StartDoc(printer, &doc);
    StartPage(printer);

    /* Draw the scene... */
    printf("Rendering scene at %dx%d pixels...\n", Width, Height);

    Redraw();

    puts("Scene drawn; sending to printer...");
    StretchBlt(printer, 0, 0, Width * 4, Height * 4, dc, 0, 0, Width, Height,
               SRCCOPY);
