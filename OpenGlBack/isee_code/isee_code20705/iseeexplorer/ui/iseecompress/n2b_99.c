...
#if 0
#define N       (128*1024)          /* size of ring buffer */
#else
#define N       (1024*1024)         /* size of ring buffer */
#define SWD_USE_MALLOC
#define SWD_HSIZE   65536
#endif
...
static void
code_prefix_ss11(UCL_COMPRESS_T *c, ucl_uint32 i)
{
    if (i >= 2)
    {
        ucl_uint32 t = 4;
...
#if defined(NRV2D)
static void
code_prefix_ss12(UCL_COMPRESS_T *c, ucl_uint32 i)
{
    if (i >= 2)
    {
        ucl_uint32 t = 2;
...
static void
code_match(UCL_COMPRESS_T *c, ucl_uint m_len, ucl_uint m_off)
{
    ucl_uint m_low = 0;
    ...
    else
    {
        ...
        bbPutByte(c, (((m_off - 1) & 0x7f) << 1) | ((m_low > 1) ? 0 : 1));
        bbPutBit(c, m_low & 1);
        c->last_m_off = m_off;
        if (m_off > c->result[1])
            c->result[1] = m_off;
...
#if !defined(swd)
    swd = (ucl_swd_p) ucl_alloc_hook(1, ucl_sizeof(*swd));
    if (!swd)
...
lazy_match_done: ;
    ...
    bbPutBit(c, 0);
#if defined(NRV2B)
    code_prefix_ss11(c, 0x1000000L);
    bbPutByte(c, 0xff);
#elif defined(NRV2D)
    code_prefix_ss12(c, 0x1000000L);
    bbPutByte(c, 0xff);
#endif
    bbFlushBits(c, 0);
