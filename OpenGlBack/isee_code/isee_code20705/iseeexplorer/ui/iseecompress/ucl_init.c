...
#  if (UCL_BYTE_ORDER == UCL_LITTLE_ENDIAN)
        r &= __ucl_assert(a[0] == 0x03020100L);
        r &= __ucl_assert(a[1] == 0x04030201L);
        r &= __ucl_assert(a[2] == 0x05040302L);
        r &= __ucl_assert(a[3] == 0x06050403L);
#  elif (UCL_BYTE_ORDER == UCL_BIG_ENDIAN)
        r &= __ucl_assert(a[0] == 0x00010203L);
        r &= __ucl_assert(a[1] == 0x01020304L);
        r &= __ucl_assert(a[2] == 0x02030405L);
        r &= __ucl_assert(a[3] == 0x03040506L);
#  endif
    }
#endif
    ...
    r &= __ucl_assert(ucl_sizeof_dict_t == sizeof(ucl_dict_t));
...
UCL_PUBLIC(int)
__ucl_init2(unsigned v, int s1, int s2, int s3, int s4, int s5,
