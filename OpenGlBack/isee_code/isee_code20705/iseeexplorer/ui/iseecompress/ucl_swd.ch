...
#    define HEAD2(b,p)      (* (ucl_ushortp) &(b[p]))
#  else
...
#if defined(SWD_USE_MALLOC)
    s->b = ucl_alloc_hook(s->n + s->f + s->f, 1);
    s->head3 = ucl_alloc_hook(SWD_HSIZE, sizeof(*s->head3));
    s->succ3 = ucl_alloc_hook(s->n + s->f, sizeof(*s->succ3));
    s->best3 = ucl_alloc_hook(s->n + s->f, sizeof(*s->best3));
    s->llen3 = ucl_alloc_hook(SWD_HSIZE, sizeof(*s->llen3));
    if (!s->b || !s->head3  || !s->succ3 || !s->best3 || !s->llen3)
        return UCL_E_OUT_OF_MEMORY;
#ifdef HEAD2
    s->head2 = ucl_alloc_hook(65536L, sizeof(*s->head2));
    if (!s->head2)
        return UCL_E_OUT_OF_MEMORY;
#endif
#endif
...
#ifdef HEAD2
    ucl_free_hook(s->head2); s->head2 = NULL;
#endif
    ucl_free_hook(s->llen3); s->llen3 = NULL;
    ucl_free_hook(s->best3); s->best3 = NULL;
    ucl_free_hook(s->succ3); s->succ3 = NULL;
    ucl_free_hook(s->head3); s->head3 = NULL;
    ucl_free_hook(s->b); s->b = NULL;
#else
    UCL_UNUSED(s);
#endif
}
