 ...
 *
 *  $Id: jbig.c,v 1.15 2002-04-09 11:13:03+01 mgk25 Exp $
 *
 ...
 *     （jbig kit原始代码中的内存分配部分是不安全的，当前这个文件已对其中
 *       的读图象函数进行了修补，可安全使用。但保存图象函数未修补）
 *
...
#define MX_MAX  23     /* maximal supported mx offset for * adaptive template in the encoder */
...
/* NEW */
  ...
  assert(s);
  memset((void*)s, 0, sizeof(struct jbg_dec_state));
/* <<<<<<<<<<<<<<<<<<< modify end */
...
/* NEW */
  ...
  __try
  {
	  ...
	  {
/* <<<<<<<<<<<<<<<<<<< modify end */
/* 注：下面try块代码中的return语句都经过了改造，原代码为直接return错误码 */
/*     这样做主要是预防内存分配函数执行失败的情况 */
		  if (!cnt) cnt = &dummy_cnt;
		  *cnt = 0;
		  if (len < 1){ result = JBG_EAGAIN; __leave; }
