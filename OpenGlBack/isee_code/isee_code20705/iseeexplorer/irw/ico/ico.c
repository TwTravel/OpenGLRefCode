...
IRWP_INFO			ico_irwp_info;			/* 插件信息表 */

#ifdef WIN32
CRITICAL_SECTION	ico_get_info_critical;	/* ico_get_image_info函数的关键段 */
CRITICAL_SECTION	ico_load_img_critical;	/* ico_load_image函数的关键段 */
CRITICAL_SECTION	ico_save_img_critical;	/* ico_save_image函数的关键段 */
#else
/* Linux对应的语句 */
#endif
...
int CALLAGREEMENT _read_icon_header(ISFILE *file, ICONDIR **icon);
...
int CALLAGREEMENT _conv_image_block(LPBITMAPINFOHEADER lpbmi, void *lpdest, enum ICO_DATA_TYPE mark);
...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
    switch (ul_reason_for_call)
	{
		case DLL_PROCESS_ATTACH:
			/* 初始化插件信息表 */
			_init_irwp_info(&ico_irwp_info);

			/* 初始化访问关键段 */
			InitializeCriticalSection(&ico_get_info_critical);
			InitializeCriticalSection(&ico_load_img_critical);
			InitializeCriticalSection(&ico_save_img_critical);
		...
		case DLL_PROCESS_DETACH:
			/* 销毁访问关键段 */
			DeleteCriticalSection(&ico_get_info_critical);
			DeleteCriticalSection(&ico_load_img_critical);
			DeleteCriticalSection(&ico_save_img_critical);
			break;
    }
    return TRUE;
}
...
ICO_API LPIRWP_INFO CALLAGREEMENT is_irw_plugin()
{
	return (LPIRWP_INFO)&ico_irwp_info;
}

#else

ICO_API LPIRWP_INFO CALLAGREEMENT ico_get_plugin_info()
{
	_init_irwp_info(&ico_irwp_info);
...
ICO_API void CALLAGREEMENT ico_init_plugin()
{
	/* 初始化多线程同步对象 */
}

ICO_API void CALLAGREEMENT ico_detach_plugin()
{
	/* 销毁多线程同步对象 */
}
...
/* 获取图像信息 */
ICO_API int CALLAGREEMENT ico_get_image_info(PISADDR psct, LPINFOSTR pinfo_str)
{
	...
	__try
	{
		...
		{
			/* 进入关键段 */
			EnterCriticalSection(&ico_get_info_critical);
			...
			/* 读取文件头信息 */
			switch (_read_icon_header(pfile, (ICONDIR **)&lpir))
			{
			case	-1:		/* 文件读写失败 */
		...
		__finally
		{
			...
			LeaveCriticalSection(&ico_get_info_critical);
		}
	}
...
/* 读取图像位数据 */
ICO_API int CALLAGREEMENT ico_load_image(PISADDR psct, LPINFOSTR pinfo_str)
{
	ISFILE			*pfile = (ISFILE*)0;
	LPICONDIR		lpir = 0;
	...
	__try
	{
		__try
		{
			EnterCriticalSection(&ico_load_img_critical);
			...
			/* 读首帧图象到数据包中 */
			switch (_conv_image_block(lpbmi, (void *)pinfo_str, ICO_PRI_IMAGE))
			{
			case	-2:		/* 图标文件受损 */
				b_status = ER_BADIMAGE;  __leave; break;
			case	-3:		/* 内存不足 */
			...
			for (i=0;i<(int)(pinfo_str->imgnumbers-1)/*主图象不算*/;i++)
			{
				...
				/* 转换子图象到数据包中 */
				switch (_conv_image_block(lpbmi, (void *)lpsub, ICO_SUB_IMAGE))
				{
				case	-2:		/* 图标文件受损 */
					b_status = ER_BADIMAGE;  __leave; break;
				case	-3:		/* 内存不足 */
		...
		__finally
		{
			...
			LeaveCriticalSection(&ico_load_img_critical);
		}
	}
	__except(EXCEPTION_EXECUTE_HANDLER)
	{
...
/* 保存图像 */
ICO_API int CALLAGREEMENT ico_save_image(PISADDR psct, LPINFOSTR pinfo_str, LPSAVESTR lpsave)
{
	enum EXERESULT	b_status = ER_SUCCESS;
	...
	__try
	{
		__try
		{
			EnterCriticalSection(&ico_save_img_critical);
	
			b_status = ER_NOTSUPPORT;
		}
		__finally
		{
			LeaveCriticalSection(&ico_save_img_critical);
		}
	}
	__except(EXCEPTION_EXECUTE_HANDLER)
	{
...
int CALLAGREEMENT _read_icon_header(ISFILE *file, ICONDIR **icon)
{
	...
	__try
	{
		...
		/* 分配用于存放图标信息的缓冲区内存块 */
		if ((lpir=(LPICONDIR)malloc(infosize)) == 0)
		{
		...
		{
			result = -2; __leave;		/* 图标文件受损（也有可能是文件读写错，但图象受损的可能性更大） */
		}
	}
	__finally
	{
	...
	*icon = lpir;	/* 当发生错误或有异常时，*icon将被置 0 */
	...
	return result;	/* 返回 0 成功，非 0 值失败。*icon指向的内存块在不需要时应释放。 */
}
...
/* 读取一个图标的象素数据块（包含BITMAPINFOHEADER结构、XOR数据和AND数据）*/
int CALLAGREEMENT _read_image_block(ISFILE *file, unsigned long offset, unsigned long len, unsigned char **image)
{
	unsigned char *	tmp = 0;
	int				result = 0;
