...
BOOL DrawTo(HDC hDC,LPRECT rc_dest,LPRECT rc_src,LPVOID lpData)
{
   ...
   m_bmif.bmiHeader.biWidth = rc_src->right - rc_src->left;
   m_bmif.bmiHeader.biHeight = rc_src->bottom - rc_src->top;

   return DrawDibDraw ( m_hDrawDib, hDC,rc_dest->left,rc_dest->top,rc_dest->right-rc_dest->left,rc_dest->bottom-rc_dest->top,
		&(m_bmif.bmiHeader), lpData,rc_src->left,rc_src->top,rc_src->right - rc_src->left,rc_src->bottom - rc_src->top,DDF_HALFTONE);
}
